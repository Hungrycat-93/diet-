...
@st.cache_data
def load_data():
    excel_path = os.path.join(os.path.dirname(__file__), "식단매크로.xlsx")
    return pd.read_excel(excel_path, sheet_name="식품명단").dropna(subset=["식품"])

df = load_data()

st.title("🥗 식단 설계기 v0.2 (FIXED)")
...
if abs(diff) <= 50:
    ...
    for food, kcal_per_unit, qty, food_kcal in sorted(food_details, key=lambda x: -x[3]):
        if kcal_per_unit > 0:
            reduce_qty = round(diff / kcal_per_unit * 10) / 10
            if reduce_qty < qty:
